...
def _gcm_send_plain(registration_id, data, collapse_key=None, delay_while_idle=False, time_to_live=0, key=None):
	"""
...
def _gcm_send_json(registration_ids, data, collapse_key=None, delay_while_idle=False, time_to_live=0, key=None):
	"""
	...
	data = json.dumps(values, separators=(",", ":"), sort_keys=True).encode("utf-8")  # keys sorted for tests
        ...
        if key:
            result = json.loads(_gcm_send(data, "application/json", key))
        else:
...
def gcm_send_message(registration_id, data, collapse_key=None, delay_while_idle=False, time_to_live=0, key=None):
	"""
	Sends a GCM notification to a single registration_id.
	...
	args = data, collapse_key, delay_while_idle, time_to_live, key
...
def gcm_send_bulk_message(registration_ids, data, collapse_key=None, delay_while_idle=False, time_to_live=0, key=None):
	"""
	Sends a GCM notification to one or more registration_ids. The registration_ids
	needs to be a list.
	This will send the notification as json data.
